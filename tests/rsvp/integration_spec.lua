...
describe("rsvp integration", function()
  before_each(function()
    ...
    -- Ensure we are in a clean modifiable buffer
  ...
  it("starts RSVP and renders the first word", function()
    ...
    for _, line in ipairs(lines) do
      if line:match("one") then found = true end
    end
  ...
  it("adjusts WPM via keymaps", function()
    ...
    local k_map, j_map
    for _, map in ipairs(keymaps) do
        if map.lhs == "k" then k_map = map end
        if map.lhs == "j" then j_map = map end
    end
    
    -- Initial WPM 300
    assert.truthy(vim.api.nvim_buf_get_lines(buf, 0, 1, false)[1]:match("WPM: 300"))
    
    if k_map.callback then k_map.callback() end -- +50
    assert.truthy(vim.api.nvim_buf_get_lines(buf, 0, 1, false)[1]:match("WPM: 350"))
    
    if j_map.callback then j_map.callback() end -- -50
    assert.truthy(vim.api.nvim_buf_get_lines(buf, 0, 1, false)[1]:match("WPM: 300"))
  end)
  ...
  it("resets progress via keymap", function()
    ...
    for _, map in ipairs(keymaps) do
        if map.lhs == " " then space_map = map end
        if map.lhs == "r" then r_map = map end
    end
    
    if space_map.callback then space_map.callback() end -- Start playing
    -- State should now be at word 2 or further if timer fired (but timer is async)
    
    if r_map.callback then r_map.callback() end -- Reset
